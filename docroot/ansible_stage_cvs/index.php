...
###  Project Manager
#
# Version : $Id: project_manager.php,v 1.1 2010/11/18 21:55:54 dave Exp $
#
#########################
...
###  Globals
$PROJECT_PROJECT_TIMERS = false;
$SYSTEM_PROJECT_BASE = '/shared/accounts/m/mrsfields2/projects';
$PROJECT_SAFE_BASE = '/shared/accounts/m/mrsfields2/projects/logs';
$_SERVER['PROJECT_CVS_BASE'] = realpath( dirname(__FILE__) .'/../../'); # Just get it from out relative location
$CVS_CMD =     'cd ' .$_SERVER['PROJECT_CVS_BASE']. ';    export CVS_RSH=ssh; sudo -u project /usr/bin/cvs -d ":ext:project@dev.project.o:/sandbox/cvsroot"';
if ( is_dir("/sandbox/cvsroot") ) $CVS_CMD =        'cd ' .$_SERVER['PROJECT_CVS_BASE']. ';    export CVS_RSH=ssh; sudo -u project /usr/bin/cvs -d "/sandbox/cvsroot"';
$CVS_CMD_4CO = 'cd ' .$_SERVER['PROJECT_CVS_BASE']. '/..; export CVS_RSH=ssh; sudo -u project /usr/bin/cvs -d ":ext:project@dev.project.o:/sandbox/cvsroot"';
if ( is_dir("/sandbox/cvsroot") ) $CVS_CMD_4CO    = 'cd ' .$_SERVER['PROJECT_CVS_BASE']. '/..; export CVS_RSH=ssh; sudo -u project /usr/bin/cvs -d "/sandbox/cvsroot"';
$cvs_cache = array();
$MAX_BATCH_SIZE = 500;
...
if ( preg_match('/[^\w\_\-\.]/', $tag, $m) ) 
    ...
    foreach ( get_affected_files($project_name) as $file ) {
        ...
        else if ( ! is_dir($_SERVER['PROJECT_CVS_BASE'] ."/$parent_dir") ) {
            $my_file = $file;
    ...
    $command_output = '';
    if ( $do_update ) {
        $update_cmd = "$CVS_CMD ". join(' ', $update_cmd);
        if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
        log_cvs_action($update_cmd);
        $command_output .= `$update_cmd 2>&1 | cat -`;
        if ( $PROJECT_PROJECT_TIMERS ) END_TIMER('CVS_CMD');
        $cmd .= "\n".( length($cmd) ? ' ; ' : ''). $update_cmd;
    }

    ###  Run the CHECKOUT command (if any)
    if ( $do_checkout ) {
        $checkout_cmd = "$CVS_CMD_4CO ". join(' ', $checkout_cmd);
        if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
    ...
    foreach ( get_affected_files($project_name) as $file ) {
        ...
        if ( ! is_dir($_SERVER['PROJECT_CVS_BASE'] ."/$parent_dir") || ! is_file($_SERVER['PROJECT_CVS_BASE'] ."/$file") ) {
    ...
    if ( $do_remove_tag ) {
        $remove_tag_cmd = "$CVS_CMD ". join(' ', $remove_tag_cmd);
        if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
...
ENDHTML;
    ...
    foreach ( $files as $file ) {
        ...
        ###  Get Current Version
        if ( ! file_exists($_SERVER['PROJECT_CVS_BASE'] ."/$file") ) {
            $cur_vers = '<i>-- n/a --</i>';
        } else if ( is_dir($_SERVER['PROJECT_CVS_BASE'] ."/$file") ) {
            $cur_vers = '<i>Directory</i>';
        } else {
            $cstat = get_cvs_status($file);;
...
function part_log_page() {
    ...
    echo "<h2>cvs log entries of $file from -r $from_rev to -r $to_rev</h2>\n<p><a href=\"javascript:history.back()\">Go Back</a></p>\n<hr>\n\n";
...
function diff_page() {
    ...
    echo "<h2>cvs diff of $file from -r $from_rev to -r $to_rev</h2>\n<p><a href=\"javascript:history.back()\">Go Back</a></p>\n<hr>\n\n";

    ###  Get the partial diff
    $to_rev_clause = ($to_rev == 'local' ? "" : "-r $to_rev");
    if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
    $cdiff = `$CVS_CMD diff -bc -r $from_rev $to_rev_clause "$file" 2>&1 | cat`;
    if ( $PROJECT_PROJECT_TIMERS ) END_TIMER('CVS_CMD');
...
function get_cvs_log( $file ) {

    ###  If not cached, get it and cache
    if ( ! $cvs_cache['log'][$file] ) {
        $parent_dir = preg_replace('@/[^/]+$@','',$file);
        if ( is_dir($_SERVER['PROJECT_CVS_BASE'] ."/$parent_dir") ) {
            if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
...
function cache_cvs_logs( $files ) {
    ...
    while ( count($files) > 0 ) {
        ...
        while ( $files && $round < $MAX_BATCH_SIZE && length($round_str) < $MAX_BATCH_STRING_SIZE ) {
            ...
            $parent_dir = preg_replace('@/[^/]+$@','',$file);
            if ( ! is_dir($_SERVER['PROJECT_CVS_BASE'] ."/$parent_dir") ) continue;
        ...
        if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
        $all_entries = `$CVS_CMD log $round_str 2>&1 | cat`;
...
function cache_cvs_statuses( $files ) {
    ...
    while ( count($files) > 0 ) {
        ...
        $round_checkoff = array_flip( $round );
        if ( $PROJECT_PROJECT_TIMERS ) START_TIMER('CVS_CMD');
        $all_entries = `$CVS_CMD status $round_str 2>&1 | cat`;
#        bug substr($all_entries, -200);
        if ( $PROJECT_PROJECT_TIMERS ) END_TIMER('CVS_CMD');
        foreach ( preg_split('@===================================================================+\n@', $all_entries) as $entry ) {
            if ( preg_match('/^\s*$/s', $entry, $m) ) continue;
...
function log_cvs_action( $command ) {

    $log_line = join_delim_line(',', array(time(), get_pid(), date(DATE_RFC822,localtime()), $command), '"'). "\n";

    $file = "$PROJECT_SAFE_BASE/project_cvs_log_".$env_mode.".csv";
...
function env_header() {
    ...
    $ret .= "<br><b>Current Sandbox Root</b>: ". $_SERVER['PROJECT_CVS_BASE'];
