...
class PhoneySavePoint {
    ...
    public function __construct($the_dbh = null) {
        ...
        try {
			...
			bug( $this->dbh->getAttribute( PDO::ATTR_DRIVER_NAME ) );
			if ( $this->dbh->getAttribute( PDO::ATTR_DRIVER_NAME ) == 'PG' ) {
				$this->dbh->exec("SAVEPOINT ". $this->savepoint_id);
			}
			else $this->phoney = true;
        }
    }
    public function commit()   { if (! $this->closed) { if ($this->started_trans) $this->dbh->commit();    else if (!$this->phoney) $this->dbh->exec("RELEASE SAVEPOINT ".     $this->savepoint_id);  $this->closed = true;  return true; } else { return false; } }
    public function rollback() { if (! $this->closed) { if ($this->started_trans) $this->dbh->rollback();  else if (!$this->phoney) $this->dbh->exec("ROLLBACK TO SAVEPOINT ". $this->savepoint_id);  $this->closed = true;  return true; } else { return false; } }
